...
class Manager {
  ...
  _runSanityCheck (client, binPath) {
    ...
    .then(() => {
      ...
      // th3brink: return this._spawn(binPath, sanityCheck.args);
      // th3brink: FIX posqd so version doesn't return 1
      // th3brink: posqd isn't finishing properly and is breaking the sanity check.
      return Promise.resolve({
        stdout: 'POSQ Core Daemon version -dirty', // POSQ Core Daemon 0.16.0.5 | POSQ Core Daemon version -dirty
        stderr: ''
      })
    })
    .then((output) => {
