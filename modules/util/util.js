...
function posqConfFilePath(testnet) {
  if (testnet) {
    return getRoamingTestNetPath("posq.conf");
  }
  return getRoamingPOSQPath("posq.conf");
}
exports.posqConfFilePath = posqConfFilePath;

function getWalletDatFilePath(testnet, name) {
  if (testnet) {
    return getRoamingTestNet3Path(name || "wallet.dat");
  }
  return getRoamingPOSQPath(name || "wallet.dat");
}
...
function getFolderPath(testnet, folder) {
  ...
  return getRoamingPOSQPath(folder);
}
exports.getFolderPath = getFolderPath;

function getRoamingPOSQPath(path) {
  switch (process.platform) {
    case 'win32':
      return process.env.APPDATA + "\\POSQ\\" + (path || "");
    case 'darwin':
      return process.env.HOME + "/Library/Application\ Support/POSQ/" + (path || "");
    default:
      return process.env.HOME + "/.posq/" + (path || "");
  }
}
exports.getRoamingPOSQPath = getRoamingPOSQPath;
...
function getRoamingPOSQDesktopPath(path) {
  switch (process.platform) {
    case 'win32':
      return process.env.APPDATA +"\\POSQ\ Desktop\\" + (path || "");
    case 'darwin':
      return process.env.HOME + "/Library/Application\ Support/POSQ\ Desktop/" + (path || "");
    default:
      return process.env.HOME + "/.config/posq-desktop/" + (path || "");
  }
}
exports.getRoamingPOSQDesktopPath = getRoamingPOSQDesktopPath;

function getRoamingTestNetPath(path) {
  switch (process.platform) {
    case 'win32':
      return process.env.APPDATA + "\\POSQTEST\\" + (path || "");
    case 'darwin':
      return process.env.HOME + "/Library/Application\ Support/POSQTEST/" + (path || "");
    default:
      return process.env.HOME + "/.posqtest/" + (path || "");
  }
}
...
function getRoamingTestNet3Path(path) {
  switch (process.platform) {
    case 'win32':
      return process.env.APPDATA + "\\POSQTEST\\testnet3\\" + (path || "");
    case 'darwin':
      return process.env.HOME + "/Library/Application\ Support/POSQTEST/testnet3/" + (path || "");
    default:
      return process.env.HOME + "/.posqtest/testnet3/" + (path || "");
  }
}
...
function removeDaemon() {
  const posqdPath = getRoamingPOSQDesktopPath('posqd');
  deleteFolderRecursive(posqdPath);
}
...
function getSettingValue(setting) {
  return getConfValue(getRoamingPOSQDesktopPath('Settings'), setting, undefined, true);
}
