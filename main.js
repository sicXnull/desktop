...
if (process.platform === 'linux') {
  app.setName('posq-desktop');
  app.setPath('userData', `${app.getPath('appData')}/${app.getName()}`);
}

const roamingDtPath = util.getRoamingPOSQDesktopPath();
const posqdPath = util.getRoamingPOSQDesktopPath('posqd');
const settingsPath = util.getRoamingPOSQDesktopPath('Settings');
let client = "";
...
function initMainWindow() {
  ...
  // th3brink: Added for debugging integrations of posq && Remove after integration
    ...
    if(!app.isQuiting && !app.isDaemonStopping){
      ...
      if (isMac && !globalStore.get('wallet:unlocked-for-staking')) { // skip confirmation
        ...
        return dialog.showMessageBox({
          ...
          title: 'Confirm',
          icon: path.join(__dirname, 'resources/icon.png'),
          message: 'Are you sure you want to close POSQ?'
        }, function (response) {
          if (response === 1) { // Exit
            close();
          }
      ...
      dialog.showMessageBox({
        ...
        title: 'Confirm',
        icon: path.join(__dirname, 'resources/icon.png'),
        message: 'How would you like to close POSQ?'
      }, function (response) {
        if (response === 2) { // Exit
          close();
...
function makeTray() {
      ...
      submenu: [
        {
          ...
          click() { electron.shell.openExternal('https://www.posqproject.org/#about'); }
        },
        {
          label: 'Visit POSQ.io',
          click() { electron.shell.openExternal('https://www.posqproject.org'); }
        },
        {
  ...
  // Set the tray icon
  _tray.setToolTip('POSQ ' + app.getVersion());
