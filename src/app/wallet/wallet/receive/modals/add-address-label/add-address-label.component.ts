...
import { RpcService, SnackbarService, POSQService } from '../../../../../core';
...
export class AddAddressLabelComponent implements OnInit {
  ...
  addNewLabel(): void {
    ...
    if (this.address !== '') {
      ...
      this.posq.addAddressLabel(this.address, this.label);
      this.posq.addAddressCreationDate(this.address);
      ...
      this.rpc.call(call, callParams)
        .subscribe(response => {
          if (['getnewaddress', 'getnewstealthaddress'].includes(call)) {
            this.posq.addAddressLabel(response, this.label);
            this.posq.addAddressCreationDate(response);
          }
          this.onAddressAdd.emit(response);
          this.dialogRef.close();
          this.flashNotificationService.open(msg)
        }, error => this.log.er('addNewLabel: ', error));
    }
  }
