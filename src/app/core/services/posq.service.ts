...
export class POSQService {
  ...
  callPOSQMap(method: string, params?: Array<any> | null): [string | null, any] {
    switch (method) {
      case 'getstakinginfo':
        return ['getstakingstatus', params];
      case 'filteraddresses':
        return this.posqFilterAddresses(params);
      case 'getnewaddress':
      ...
      case 'manageaddressbook':
        const addresses = this.posqFilterAddressBook(params);
        return [null, addresses];
      case 'sendtypeto':
        return this.posqSendTypeTo(params);
    }
    return [method, params];
  }

  callGroupPOSQMap(method: string, groupParams: Array<any>, params?: Array<any> | null): [string | null, any][] {
    switch (method) {
      default:
  ...
  responsePOSQMap(method: string, params?: Array<any> | null, response?: any): any {
    ...
    switch (method) {
      ...
      case 'listtransactions':
        ...
        return Promise.resolve(response);
      case 'getwalletinfo': 
        return Promise.resolve(this.posqWalletInfo(params, response));
      case 'encryptwallet': 
  ...
  responseGroupPOSQMap(method: string, params?: Array<any> | null, responses?: any[]): any {
    switch (method) {
      default:
  ...
  posqWalletInfo(params: any[], waletInfo: any): any {
  ...
  posqFilterAddressBook(params: any[]) {
  ...
  posqFilterAddresses(params: any[]): [string | null, any] {
  ...
  posqSendTypeTo(params: any[]): [string | null, any] {
    const [
      typeIn,
      typeOut,
      outputs,
  ...
  call(method: string, params?: Array<any> | null): Observable<any> {
    const [mMethod, mParams] = this.callPOSQMap(method, params);
    if (!mMethod) {
    ...
    if (this.isElectron) {
      return this._ipc.runCommand('rpc-channel', null, mMethod, mParams)
        .pipe(
          mergeMap(response => fromPromise(this.responsePOSQMap(method, params, response))),
  ...
  callGroup(method: string, groupParams: any[], params?: any[] | null): Observable<any> {
    const reqs = this.callGroupPOSQMap(method, groupParams, params);
    ...
    return Observable.forkJoin(requests).pipe(
      mergeMap(response => fromPromise(this.responseGroupPOSQMap(method, params, response))),
      catchError(error => Observable.throw(typeof error._body || error === 'object'
        ? error._body || error
        : JSON.parse(error._body || error))
      )
